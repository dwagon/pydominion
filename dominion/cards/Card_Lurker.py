#!/usr/bin/env python
"""https://wiki.dominionstrategy.com/index.php/Lurker"""
import unittest
...
class Card_Lurker(Card.Card):
    ...
    def special(self, game: "Game.Game", player: "Player.Player") -> None:
        if player.plr_choose_options(
            ...
            lurker_trash_supply(game, player)
        else:
            lurker_from_trash(game, player)


###############################################################################
def lurker_trash_supply(game: "Game.Game", player: "Player.Player") -> None:
    """Trash an action from supply"""
    options = []
    for name, pile in game.get_card_piles():
        if pile.is_empty() or not game.card_instances[name].insupply:
            continue
...
###############################################################################
def lurker_from_trash(game: "Game.Game", player: "Player.Player") -> None:
    ...
    if cards := player.card_sel(cardsrc=acts, prompt="Select Action from the Trash"):
        player.move_card(cards[0], Piles.DISCARD)
...
class TestLurker(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.card = self.plr.get_card_from_pile("Lurker")
